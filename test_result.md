...
agent_communication:
  - agent: "main"
    message: "🚀 EVOLUTION API INSTANCE CREATION COMPLETELY REVAMPED (2025-01-21): Implemented comprehensive instance creation following official Evolution API documentation. Key enhancements: (1) Added rejectCall=true to automatically reject all incoming calls (2) Added groupsIgnore=true to ignore all group messages (3) Added alwaysOnline=true to keep WhatsApp always online (4) Implemented full webhook configuration with MESSAGES_UPSERT enabled for receiving incoming messages (5) Added complete event handling for all Evolution API events (6) Enhanced webhook endpoint /api/webhook/evolution for processing webhook events (7) Added proper error handling and logging. Instance creation now matches user requirements exactly as specified in the continuation request. Ready for testing."
  - agent: "testing"
